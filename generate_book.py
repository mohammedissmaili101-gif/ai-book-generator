...
Optimized with Two-Step Generation and Robust Error Handling.
"""
...
BOOK_THEMES = {
    ...
    "health": {
        "primary": "#052e16", "secondary": "#14532d", "accent": "#4ade80",
        "text": "#f0fdf4", "highlight": "#86efac", "font_title": "Merriweather",
        "font_body": "Source Serif Pro", "emoji": "🌿",
    ...
    "finance": {
        "primary": "#0a0a0a", "secondary": "#111", "accent": "#ffd700",
        "text": "#e5e5e5", "highlight": "#fbbf24", "font_title": "EB Garamond",
        "font_body": "Libre Baskerville", "emoji": "💰",
    ...
    "self_help": {
        "primary": "#0a0a0a", "secondary": "#111111", "accent": "#a78bfa",
        "text": "#ede9fe", "highlight": "#c4b5fd", "font_title": "Montserrat",
    ...
    "spirituality": {
        "primary": "#1a0533", "secondary": "#2d0a4e", "accent": "#e879f9",
        "text": "#fae8ff", "highlight": "#d946ef", "font_title": "Cormorant Garamond",
        "font_body": "Crimson Text", "emoji": "🔮",
    ...
    "cooking": {
        "primary": "#1c0500", "secondary": "#431407", "accent": "#fb923c",
        "text": "#fff7ed", "highlight": "#fdba74", "font_title": "Dancing Script",
        "font_body": "Quattrocento", "emoji": "🍳",
    ...
    "travel": {
        "primary": "#0c1a2e", "secondary": "#0f2846", "accent": "#fbbf24",
        "text": "#fef3c7", "highlight": "#f59e0b", "font_title": "Josefin Sans",
        "font_body": "Raleway", "emoji": "✈️",
...
def clean_french_grammar(text) -> str:
    """Post-processing filter to fix common AI French mistakes"""
    if not text:
        return ""
    
    # Safe fallback if AI returns non-string type
    text = str(text)
...
def safe_extract_json(content: str) -> dict:
    """Robust JSON extractor to prevent crashes from AI hallucinations"""
    try:
...
Rules:
1. Return ONLY a valid JSON object. No intro/outro commentary.
2. Language of output: {language}.
Format EXACTLY as:
{{
  "title": "powerful book title",
  "subtitle": "benefit-driven subtitle",
  "author": "Maria Talks",
  "tagline": "one percutant sentence",
  "description": "200 words explaining the transition.",
  "target_audience": "young adults",
  "key_benefits": ["Benefit 1", "Benefit 2"],
  "chapters": [
    {{ "number": 1, "title": "Chapter Title", "subtitle": "Promise", "focus": "Brief" }},
    {{ "number": 2, "title": "Chapter Title", "subtitle": "Promise", "focus": "Brief" }}
  ],
  "introduction": "300 words intro.",
...
"""}
    ...
    structure = safe_extract_json(content)
    
    # Fallback to prevent crash if AI totally failed
    if not structure:
        print("⚠️ Warning: Could not generate structure. Using fallback skeleton.")
        structure = {
            "title": title, "author": "Author", 
            "chapters": [{"number": 1, "title": "Introduction", "subtitle": "", "focus": ""}]
        }

    # Clean structure texts safely
...
def generate_single_chapter_content(book_title: str, chapter: dict, language: str = "en") -> dict:
    """Step 2: Generates deep, premium content for exactly ONE chapter"""
    system_prompt = (
        "You are an elite psychological coach. You write in flawless, "
        "cold, gender-neutral, and highly strategic French. Never use fluffy clichés. "
        "Always write in short, punchy paragraphs."
    )

    user_prompt = f"""
Book: "{book_title}"
Write all the internal sections for Chapter {chapter.get('number', 1)}: "{chapter.get('title', '')}"
Chapter Promise: {chapter.get('subtitle', '')}
Focus/Brief of this Chapter: {chapter.get('focus', '')}

Output ONLY a raw, valid JSON matching this schema:
{{
  "hook": "One shocking opening hook.",
  "key_points": ["Point 1", "Point 2"],
  "content": "A detailed 800-word analysis.",
  "key_technique": "Named technique.",
  "exercises": ["Exercise 1", "Exercise 2"],
  "summary": "Closing takeaway sentence."
}}
"""
    messages = [
    ...
    ch_data = safe_extract_json(content)
    
    # Post-process grammar cleaning safely
...
def generate_pdf_html(book_data: dict, theme: dict, language: str = "en") -> str:
    ...
    for i, chapter in enumerate(book_data.get("chapters", []), 1):
        ...
        raw_content = str(chapter.get("content", ""))
...
<section class="chapter" id="chapter-{i}">
    ...
    # Safe extraction of author initials
    author_name_str = book_data.get("author", "Professional Author")
    author_initial = author_name_str[0].upper() if author_name_str else "A"
    ...
    intro_html = "".join(f'<p class="body-text">{p.strip()}</p>' for p in str(book_data.get("introduction", "")).split('\n') if p.strip())
    conc_html = "".join(f'<p class="body-text">{p.strip()}</p>' for p in str(book_data.get("conclusion", "")).split('\n') if p.strip())

    title_short = str(book_data.get('title', ''))
...
.cover-page {{
    height: 297mm; box-sizing: border-box; background: var(--cover-gradient); display: block;
    padding-top: 85mm; padding-left: var(--side); padding-right: var(--side); position: relative; overflow: hidden;
}}
...
.cover-circle-big {{ position: absolute; top: 30px; left: 50%; transform: translateX(-50%); width: 200px; height: 200px; border-radius: 50%; border: 40px solid var(--cover-accent); opacity: 0.06; }}
...
.cover-emoji {{ font-size: 64px; margin-bottom: 28px; display: block; filter: drop-shadow(0 8px 24px rgba(0,0,0,0.5)); }}
.cover-line {{ width: 60px; height: 4px; background: var(--cover-accent); border-radius: 2px; margin-bottom: 20px; }}
.cover-title {{ font-family: var(--font-title); font-size: clamp(1.9rem, 6vw, 2.6rem); font-weight: 900; color: #fff; line-height: 1.15; margin-bottom: 14px; text-shadow: 0 2px 20px rgba(0,0,0,0.5); }}
.cover-subtitle {{ font-size: var(--size-sm); color: var(--cover-accent); font-style: italic; margin-bottom: 36px; opacity: 0.9; }}
.cover-author-row {{ display: flex; align-items: center; gap: 10px; padding-top: 24px; border-top: 1px solid rgba(255,255,255,0.12); }}
...
.description-text {{ font-size: var(--size-base); color: rgba(255,255,255,0.78); margin-bottom: var(--gap-lg); }}
.benefits-list {{ display: flex; flex-direction: column; gap: 10px; }}
.benefit-item {{ display: flex; align-items: flex-start; gap: 12px; padding: 13px 15px; background: rgba(255,255,255,0.04); border-radius: var(--radius); border-{border_side}: 3px solid var(--accent); }}
...
.intro-page {{ padding: var(--gap-xl) var(--side); background: var(--secondary); position: relative; }}
.chapter-wrapper {{ display: block; position: relative; }}
.chapter {{ border-bottom: 3px solid var(--accent); }}
.chapter-header {{ background: var(--gradient); padding: var(--gap-xl) var(--side) var(--gap-lg); text-align: center; position: relative; overflow: hidden; }}
...
.chapter-subtitle {{ color: var(--accent); font-size: var(--size-sm); font-style: italic; opacity: 0.88; }}
.chapter-hook {{ margin: var(--gap-md) var(--side) 0; padding: 18px 20px; border-{border_side}: 4px solid var(--accent); background: rgba(255,255,255,0.03); font-style: italic; font-size: var(--size-base); color: rgba(255,255,255,0.75); line-height: 1.75; }}
.chapter-body {{ padding: var(--gap-md) var(--side) var(--gap-lg); }}
.body-text {{ display: block; font-size: var(--size-base); line-height: 1.9; color: rgba(255,255,255,0.82); margin-bottom: 18px; word-break: break-word; overflow-wrap: break-word; }}
.box-label {{ font-size: var(--size-xs); letter-spacing: 2px; text-transform: uppercase; margin-bottom: 14px; display: block; }}
.key-points-box {{ padding: 20px; background: rgba(255,255,255,0.03); border-radius: var(--radius); border: 1px solid rgba(255,255,255,0.08); margin-bottom: var(--gap-md); }}
.key-points-box .box-label {{ color: var(--accent); }}
.key-points-list {{ list-style: none; display: flex; flex-direction: column; gap: 8px; }}
.key-point {{ display: flex; align-items: flex-start; gap: 10px; padding: 9px 12px; background: rgba(255,255,255,0.04); border-radius: 8px; font-size: var(--size-sm); color: rgba(255,255,255,0.85); }}
.kp-arrow {{ color: var(--accent); flex-shrink: 0; }}
.technique-badge {{ display: flex; flex-direction: column; gap: 6px; padding: 16px 18px; background: linear-gradient(135deg, rgba(255,255,255,0.07), rgba(255,255,255,0.03)); border-radius: var(--radius); border: 1px solid var(--accent); margin-bottom: var(--gap-md); }}
.technique-label {{ font-size: var(--size-xs); letter-spacing: 2px; text-transform: uppercase; color: var(--accent); }}
.technique-name {{ font-family: var(--font-title); font-size: var(--size-lg); font-weight: 700; color: #fff; }}
...
.exercise-num {{ background: var(--highlight); color: var(--primary); width: 26px; height: 26px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: var(--size-xs); font-weight: 900; flex-shrink: 0; margin-top: 2px; }}
.exercise-item p {{ font-size: var(--size-sm); color: rgba(255,255,255,0.8); }}
.chapter-summary {{ display: flex; gap: 14px; align-items: flex-start; padding: 18px 20px; background: rgba(255,255,255,0.04); border-radius: var(--radius); margin-bottom: var(--gap-md); font-style: italic; }}
.summary-icon {{ font-size: 1.4rem; flex-shrink: 0; }}
.summary-text {{ font-size: var(--size-sm); color: rgba(255,255,255,0.88); }}
...
.author-card {{ background: rgba(255,255,255,0.04); border-radius: 20px; padding: 30px 24px; text-align: center; border: 1px solid rgba(255,255,255,0.08); }}
.author-avatar {{ width: 72px; height: 72px; background: var(--gradient); border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 1.8rem; margin: 0 auto 18px; border: 3px solid var(--accent); }}
.author-name {{ font-family: var(--font-title); font-size: var(--size-xl); font-weight: 700; color: #fff; margin-bottom: 6px; }}
.author-bio {{ font-size: var(--size-sm); color: rgba(255,255,255,0.65); margin-top: 12px; }}
.back-cover {{ min-height: 55vh; background: var(--cover-gradient); padding: var(--gap-xl) var(--side); display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; gap: 20px; position: relative; overflow: hidden; }}
.back-cover-title {{ font-family: var(--font-title); font-size: var(--size-2xl); font-weight: 900; color: #fff; }}
.back-cover-text {{ font-size: var(--size-sm); color: rgba(255,255,255,0.75); line-height: 1.75; max-width: 340px; }}
.edition-badge {{ background: var(--cover-accent); color: var(--primary); font-size: var(--size-sm); font-weight: 900; font-family: var(--font-title); padding: 12px 28px; border-radius: 50px; }}
#reading-progress {{ position: fixed; top: 0; left: 0; height: 3px; background: var(--accent); z-index: 999; transition: width 0.15s linear; width: 0%; }}
.pdf-btn {{ position: fixed; bottom: 22px; right: 18px; background: var(--accent); color: var(--primary); border: none; border-radius: 50px; padding: 13px 20px; font-size: var(--size-sm); font-weight: 900; cursor: pointer; z-index: 998; display: flex; align-items: center; gap: 7px; box-shadow: 0 4px 20px rgba(0,0,0,0.45); transition: transform 0.15s; font-family: var(--font-body); }}
.page-break {{ page-break-inside: avoid !important; break-inside: avoid !important; margin-bottom: 0 !important; padding-bottom: 10px !important; }}
@media print {{
    ...
    .cover-page, .description-page, .toc-page, .intro-page, .chapter-wrapper, .conclusion-page, .author-page, .back-cover {{ page-break-after: always !important; break-after: always !important; }}
}}
...
async function downloadPDF() {{
    ...
    const titleText = document.querySelector('.cover-title')?.textContent || 'book';
    const safeName = titleText.replace(/[^\\w\\s-]/g,'').trim().replace(/\\s+/g,'_').substring(0,50);
    
    const opt = {{
        margin: 0,
        filename: safeName + '.pdf',
        image: {{ type: 'jpeg', quality: 0.98 }},
        html2canvas: {{ scale: 2, useCORS: true, backgroundColor: '{theme["primary"]}', logging: false, letterRendering: true }},
...
def generate_kdp_html(book_data: dict, theme: dict, language: str = "en") -> str:
    ...
    for i, chapter in enumerate(book_data.get("chapters", []), 1):
        ...
        key_points_html = "".join(f'<li>{pt}</li>' for pt in chapter.get("key_points", []))
...
<section class="chapter">
    ...
    {content_html}
    <div class="technique-box"><p class="box-title">🔑 KEY TECHNIQUE: {chapter.get("key_technique","")}</p></div>
    <div class="exercises-box"><p class="box-title">✍️ ACTION STEPS</p>{exercises_html}</div>
    <div class="summary-box"><p>💡 <strong>Chapter Takeaway:</strong> {chapter.get("summary","")}</p></div>
</section>
</div>'''

    toc_html = "".join(f'<div class="toc-row"><span class="toc-ch">Chapter {i}: {ch.get("title","")}</span><span class="toc-dots"></span></div>' for i, ch in enumerate(book_data.get("chapters", []), 1))
    intro_html = "".join(f'<p class="body">{p.strip()}</p>' for p in str(book_data.get("introduction", "")).split('\n') if p.strip())
    conc_html = "".join(f'<p class="body">{p.strip()}</p>' for p in str(book_data.get("conclusion", "")).split('\n') if p.strip())
...
<style>
*, *::before, *::after {{ margin:0; padding:0; box-sizing:border-box; }}
:root {{ --accent: {accent}; --font-title: 'Montserrat', 'EB Garamond', serif; --font-body: 'EB Garamond', Georgia, serif; }}
html {{ scroll-behavior: smooth; }}
body {{ background: #fff; color: #111; font-family: var(--font-body); font-size: 12pt; line-height: 1.8; max-width: 152mm; margin: 0 auto; -webkit-font-smoothing: antialiased; word-wrap: break-word; }}
.cover-page {{ width: 152mm; height: 228mm; background: linear-gradient(170deg, #111 0%, #222 60%, #333 100%); display: block; padding-top: 45mm; text-align: center; padding-left: 15mm; padding-right: 15mm; position: relative; overflow: hidden; }}
.cover-page::before {{ content: ''; position: absolute; top: -50px; left: -50px; width: 250px; height: 250px; border-radius: 50%; border: 40px solid {accent}; opacity: 0.15; }}
.cover-page::after {{ content: ''; position: absolute; bottom: -30px; right: -30px; width: 180px; height: 180px; border-radius: 50%; background: {accent}; opacity: 0.08; }}
.cover-accent-line {{ width: 60px; height: 5px; background: {accent}; border-radius: 3px; margin: 20px auto; }}
.cover-emoji-kdp {{ font-size: 64px; display: block; margin-bottom: 20px; filter: drop-shadow(0 4px 20px rgba(0,0,0,0.6)); }}
.cover-title-kdp {{ font-family: var(--font-title); font-size: 2.2rem; font-weight: 900; color: #fff; line-height: 1.2; letter-spacing: -0.5px; text-shadow: 0 2px 20px rgba(0,0,0,0.5); margin-bottom: 12px; }}
.cover-subtitle-kdp {{ font-size: 1rem; color: {accent}; font-style: italic; line-height: 1.5; opacity: 0.9; margin-bottom: 30px; }}
.cover-author-kdp {{ font-family: var(--font-title); font-size: 0.85rem; letter-spacing: 4px; text-transform: uppercase; color: rgba(255,255,255,0.5); margin-top: 10px; }}
.cover-year-kdp {{ position: absolute; bottom: 20px; font-size: 0.7rem; color: rgba(255,255,255,0.2); letter-spacing: 2px; }}
.title-page {{ height: 228mm; display: block; padding-top: 60mm; text-align: center; padding-left: 20mm; padding-right: 20mm; }}
.tp-title {{ font-family: var(--font-title); font-size: 2rem; font-weight: 900; color: #111; margin-bottom: 12px; }}
.tp-subtitle {{ font-size: 1.1rem; color: #555; font-style: italic; margin-bottom: 40px; }}
.tp-rule {{ width: 80px; height: 3px; background: {accent}; margin: 0 auto 40px; border-radius: 2px; }}
.tp-author {{ font-family: var(--font-title); font-size: 1rem; letter-spacing: 3px; text-transform: uppercase; color: #333; }}
...
.section-heading {{ font-family: var(--font-title); font-size: 1.5rem; font-weight: 900; color: #111; border-bottom: 2px solid {accent}; padding-bottom: 8px; margin-bottom: 24px; }}
.toc-row {{ display: flex; align-items: baseline; padding: 8px 0; border-bottom: 1px dotted #ddd; font-size: 10.5pt; }}
.toc-ch {{ flex: 1; }} .toc-dots {{ flex: 1; }}
.intro-page, .conclusion-page {{ padding: 20mm; }}
.chapter {{ padding: 18mm 20mm 20mm; }}
.ch-header {{ margin-bottom: 24px; text-align: center; }}
.ch-label {{ font-family: var(--font-title); font-size: 0.65rem; letter-spacing: 4px; text-transform: uppercase; color: {accent}; margin-bottom: 8px; }}
.ch-title {{ font-family: var(--font-title); font-size: 1.8rem; font-weight: 900; color: #111; margin-bottom: 8px; }}
...
blockquote.ch-hook {{ margin: 20px 0; padding: 14px 20px; border-{border_side}: 4px solid {accent}; background: #fafafa; font-style: italic; font-size: 10.5pt; color: #444; }}
...
.box-title {{ font-family: var(--font-title); font-size: 0.8rem; font-weight: 700; color: #333; margin-bottom: 12px; }}
.exercise {{ display: flex; gap: 10px; margin-bottom: 10px; font-size: 10.5pt; color: #333; }}
.ex-num {{ font-weight: 700; color: {accent}; flex-shrink: 0; }}
.summary-box {{ background: #fff9f0; border-radius: 8px; padding: 14px 18px; border: 1px solid #f0d080; font-size: 10.5pt; color: #333; margin-top: 20px; }}
...
.back-cover-kdp {{ width: 152mm; min-height: 120mm; background: linear-gradient(170deg, #111 0%, #222 60%, #333 100%); padding: 20mm; color: #fff; position: relative; overflow: hidden; }}
...
.kdp-btn {{ position: fixed; bottom: 22px; right: 18px; background: {accent}; color: #111; border: none; border-radius: 50px; padding: 13px 20px; font-size: 0.9rem; font-weight: 900; cursor: pointer; z-index: 998; display: flex; align-items: center; gap: 7px; box-shadow: 0 4px 20px rgba(0,0,0,0.25); font-family: var(--font-title); }}
.page-break {{ page-break-before: always !important; break-before: always !important; page-break-inside: avoid !important; break-inside: avoid !important; }}
@media print {{ .kdp-btn {{ display: none !important; }} body {{ max-width: 152mm !important; width: 152mm !important; font-size: 11.5pt; background: #fff !important; }} .cover-page, .title-page, .copyright-page, .toc-page, .intro-page, .chapter-break, .conclusion-page, .author-page, .back-cover-kdp {{ page-break-after: always !important; break-after: always !important; }} }}
</style>
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
</head>
<body>
...
<section class="back-cover-kdp page-break">
    ...
    <ul class="bc-benefits">{"".join(f"<li>{b}</li>" for b in book_data.get("key_benefits",[]))}</ul>
...
async function downloadKDP() {{
    ...
    // Safely inject JS string from Python using json.dumps to avoid SyntaxErrors if title has apostrophes
    const rawTitle = {json.dumps(book_data.get("title", "book"))};
    const title = rawTitle.replace(/[^\\w\\s-]/g,'').trim().replace(/\\s+/g,'_').substring(0,50);
    ...
    const opt = {{
        ...
        jsPDF: {{ unit: 'mm', format: [152, 228], orientation: 'portrait' }},
        pagebreak: {{ mode: ['avoid-all', 'css'] }}
    }};
    try {{
        await html2pdf().set(opt).from(document.body).save();
...
[
  {{ "niche": "name", "title_idea": "idea", "why_profitable": "why", "price_range": "$X-$Y" }}
]
Language: {language}"""}
    ]
    content = call_groq(messages, max_tokens=2000).strip()
    return safe_extract_json(content) if isinstance(safe_extract_json(content), list) else []
...
def main():
    ...
    if action == "suggest":
        ...
        for ch in book_data.get("chapters", []):
            print(f"🚀 Generating Chapter {ch.get('number', '?')}: {ch.get('title', 'Sans titre')}...")
            try:
                ch_details = generate_single_chapter_content(book_title, ch, language)
                ch.update(ch_details)
            except Exception as e:
                print(f"⚠️ Error generating Chapter {ch.get('number')}: {e}. Retrying once...")
                try:
                    ch_details = generate_single_chapter_content(book_title, ch, language)
                    ch.update(ch_details)
                except Exception as e2:
                    print(f"❌ Failed twice: {e2}. Fallback to avoid crash.")
                    ch.update({"content": "Contenu indisponible suite à une erreur.", "hook": "", "summary": ""})
